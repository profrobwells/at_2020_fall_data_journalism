...
```{r}
#create an object called covid_inspections
covid_start<- "2020-03-13"
covid_end<- "2020-08-18" #date article was published
covid_inspections<-osha_analysis_data%>%
  #According to the Center for Public Integrity's repo, the "M" in insp_type refers to fatalities/ catastrophes 
  #And "S" in safety_hlth column refers to Safety inspections
    filter(safety_hlth == "S") %>%
   filter(open_date >= covid_start,
          open_date < covid_end,
          insp_scope != "D") # no non-inspections
count(covid_inspections)

#4557 inspections were conducted during covid period 

#now, let's compare that to the same period last year:
# these are same dates, just from last year
compare_start <-"2019-03-13"
compare_end<- "2019-08-18" 
# create an object called compare_inspections
compare_inspections<-osha_analysis_data%>%
    filter(safety_hlth == "S") %>%
   filter(open_date >= compare_start,
          open_date < compare_end,
          insp_scope != "D") # no non-inspections
count(compare_inspections)

#13400 inspections were conducted last year.

#4557 is approximately two-thirds of #13400, which matches the answer derived by the Center for Public Integrity.
