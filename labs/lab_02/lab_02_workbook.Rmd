...
arcos_md_baltimorecity_sorted <- arcos_md_baltimorecity_trimmed %>%
  arrange(desc(total_dosage_unit))
#ANSWER: NEWCARE HOME HEALTH SERVICES bad the most pills.
```
...
```{r}
arcos_md_baltimorecity_records_sorted <- arcos_md_baltimorecity_trimmed %>%
  arrange(desc(total_records))
#ANSWER: GIANT PHARMACY 
```
...
```{r}
arcos_md_baltimorecity_pills <- arcos_md_baltimorecity_trimmed %>%
  arrange(total_dosage_unit)
#ANSWER: JAKOBOVITS, JULIAN MD and OSHINSKY, ALAN E MD, both got 4 pills and that was the smallest.
