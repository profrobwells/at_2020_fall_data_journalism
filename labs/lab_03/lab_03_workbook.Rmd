...
# A: Galax City, Emporia City, Northampton
...
test_filter <- covid_county_monthly_case_rate %>%
  filter(str_detect(state, "Iowa|Illinois|Ohio|Idaho"))
test_filter
```
...
# A: Butte in Idaho (infinite) or Luce in Michigan (29.25 perc)
...
covid_maryland_death_jul_aug_pct_change <- covid_county_monthly_death_rate %>%
  select(county, state, d_2020_07_30, d_2020_08_30) %>%
  filter(state == "Maryland") %>%
  mutate(covid_maryland_death_jul_aug_pct_change = (d_2020_08_30 - d_2020_07_30)/d_2020_07_30) %>%
  arrange(desc(covid_maryland_death_jul_aug_pct_change))
# Write the name of the object you just created below so it displays below in the markdown document
covid_maryland_death_jul_aug_pct_change

# Q: which Maryland county had the largest percentage change in cumulative death rate between June and August? 
# A: Warcester and Somerset
...
```{r}
covid_texas_case_may_aug_pct_change <- covid_county_monthly_case_rate %>%
  select(county, state, d_2020_05_30, d_2020_08_30) %>%
  filter(state == "Texas") %>%
  mutate(covid_texas_case_may_aug_pct_change = (d_2020_08_30 - d_2020_05_30)/d_2020_05_30) %>%
  arrange((covid_texas_case_may_aug_pct_change))

covid_texas_case_may_aug_pct_change

#ANSWER: JOnas county in texas.
```
...
#Which county in Pennsylvania had the highest percentage increase in cases from May to August?

#Answer:

```{r}
covid_pa_case_may_aug_pct_change <- covid_county_monthly_case_rate %>%
  select(county, state, d_2020_05_30, d_2020_08_30) %>%
  filter(state == "Pennsylvania") %>%
  mutate(covid_pa_case_may_aug_pct_change = (d_2020_08_30 - d_2020_05_30)/d_2020_05_30) %>%
  arrange((covid_pa_case_may_aug_pct_change))

covid_pa_case_may_aug_pct_change

#ANSWER: Pike county, PA
